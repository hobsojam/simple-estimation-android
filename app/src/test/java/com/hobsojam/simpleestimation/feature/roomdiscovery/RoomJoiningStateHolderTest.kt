...
class RoomJoiningStateHolderTest :
    FunSpec({
        test("selects an active room and requires an access PIN when the room is protected") {
            ...
            stateHolder.uiState.join.status shouldBe
                RoomJoinStatus.Error("Enter the room access PIN to continue.")
        ...
        test("reuses participant id for repeated joins to the same room in the current session") {
            ...
            stateHolder.submitJoin()
            val firstRequest =
                (stateHolder.uiState.join.status as RoomJoinStatus.ReadyToConnect).request
        ...
        test("uses separate participant ids for separate rooms in the current session") {
            ...
            stateHolder.submitJoin()
            val firstRoomRequest =
                (stateHolder.uiState.join.status as RoomJoinStatus.ReadyToConnect).request

            stateHolder.updateManualRoomInput("room-2")
            stateHolder.submitJoin()
            val secondRoomRequest =
                (stateHolder.uiState.join.status as RoomJoinStatus.ReadyToConnect).request
        ...
        test("resets participant ids when a new app session state holder is created") {
            ...
            val firstRequest =
                (firstSession.uiState.join.status as RoomJoinStatus.ReadyToConnect).request
            val secondRequest =
                (secondSession.uiState.join.status as RoomJoinStatus.ReadyToConnect).request
            firstRequest.participantId shouldBe "participant-1"
            secondRequest.participantId shouldBe "participant-2"
        }

        test(
            "keeps display name only in the current app session and does not carry access pins forward",
        ) {
        ...
        test("blocks unsupported protocol versions and preserves retry input") {
            ...
            stateHolder.uiState.join.status shouldBe RoomJoinStatus.Error(expectedError)
        ...
        test("reports network failures from config checks without clearing retry input") {
            ...
            (stateHolder.uiState.join.mode as RoomJoinMode.JoiningRoom).roomIdInput shouldBe
                "room-99"
            stateHolder.uiState.join.displayName shouldBe "Ada"
        }
        ...
        test(
            "ignores stale config response when a subsequent invalid join is submitted while in flight",
        ) {
            val deferred = CompletableDeferred<Result<ServerConfig>>()
            val stateHolder = RoomDiscoveryStateHolder(
                repository = FakeJoiningActiveRoomRepository(),
...
private class ParticipantIdSequence(private vararg val values: String) : () -> String {
    private var nextIndex = 0

    override fun invoke(): String = values[nextIndex++]
}

private class FakeServerConfigClient(private val results: List<Result<ServerConfig>>) :
    ServerConfigClient {
