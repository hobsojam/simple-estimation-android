...
class RoomDiscoveryStateHolderTest :
    FunSpec({
        ...
        test("loads active rooms from the configured server") {
            ...
            val repository =
                FakeActiveRoomRepository(ActiveRoomDiscoveryResult.Success(listOf(room)))
        ...
        test("represents an empty active-room response clearly") {
            val stateHolder = RoomDiscoveryStateHolder(
                repository = FakeActiveRoomRepository(
...
private class FakeActiveRoomRepository(private vararg val results: ActiveRoomDiscoveryResult) :
