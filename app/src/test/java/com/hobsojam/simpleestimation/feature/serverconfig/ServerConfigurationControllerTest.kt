...
class ServerConfigurationControllerTest : FunSpec({
    test("calls api config and surfaces demo mode before joining") {
        val configClient = FakeServerConfigClient(ServerConfig(demoMode = true, protocolVersion = 1))
    ...
    test("shows upgrade message for unsupported protocol versions") {
        val controller = ServerConfigurationController(
            configClient = FakeServerConfigClient(ServerConfig(demoMode = false, protocolVersion = 2)),
    ...
    test("blocks cleartext URLs when cleartext is not allowed") {
        val controller = ServerConfigurationController(
            configClient = FakeServerConfigClient(ServerConfig(demoMode = false, protocolVersion = 1)),
            cleartextAllowed = false,
        )
